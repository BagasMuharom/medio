<template>
    <div>
        ...
        >
            <template v-slot:title>
                <h1 class="text-2xl font-bold font-quicksand">
                    Selamat datang di Tic Tac Math
                </h1>
            </template>

            <template v-slot:default>
                <div class="font-quicksand">
                    Bentuk susunan bidak hingga 4 susun secara horizontal, vertikal, atau diagonal untuk memenangkan permainan.
                </div>
            </template>

            <template v-slot:footer>
                <btn-primary @click="closeModal">Mulai Sekarang</btn-primary>
            </template>
        </std-modal>
        ...
        >
            <template v-slot:default>
                <h1 class="text-4xl font-bold font-quicksand text-center">
                    Selamat !
                </h1>

                <div class="font-quicksand flex justify-center mt-4">
                    <div
                        class="w-14 h-14 rounded-md flex justify-center items-center border-4 border-yellow-400 bg-yellow-200 text-yellow-600 text-2xl"
                        v-if="pemenang == 'o'"
                    >
                        <fa-icon :icon="['far', 'circle']"></fa-icon>
                    </div>
                    <div
                        class="w-14 h-14 rounded-md flex justify-center items-center border-4 border-red-400 bg-red-200 text-red-600 text-2xl"
                        v-if="pemenang == 'x'"
                    >
            ...
            <template v-slot:footer>
                <div class="flex justify-center">
                    <div class="mr-2">
                        <btn-primary @click="playAgain">Main Lagi</btn-primary>
                    </div>

                    <btn-primary @click="backToChooseGame">Kembali</btn-primary>
                </div>
            </template>
        </std-modal>
		...
		<!-- Header -->
		<div class="flex p-3">
			<btn-primary @click="$router.push({name: 'choose.game'})">Kembali</btn-primary>
		</div>
        ...
        <div class="flex justify-center mt-4">
            <div id="number-board">
                <div
                    class="flex"
                    v-for="(row, index_row) in board"
                    :key="index_row"
                >
                    <div
                        :class="['w-12 h-12 text-center flex justify-center items-center rounded-md border-4 m-1 font-bold text-lg',
              {
                active:
            ...
            <div id="tools-board">
                <h3 class="font-bold font-quicksand">Giliran</h3>
                <div
                    class="w-14 h-14 rounded-md flex justify-center items-center border-4 border-yellow-400 bg-yellow-200 text-yellow-600 text-2xl"
                    v-if="turn == 'o'"
                >
              ...
              'border-4 font-bold rounded-md flex-grow text-2xl m-1 text-center py-2',
              {
                ...
                >
                    <div
                        ...
                        :class="[
              'border-4 flex justify-center items-center h-16 w-16 text-2xl rounded-md m-1 font-bold',
              {
